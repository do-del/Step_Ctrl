...
typedef struct{
	...
	//配置(堵转)
	#define			De_Motor_Stall	1		//默认堵转保护开关
	uint8_t				valid_stall_switch;			//堵转保护开关有效标志
	uint8_t				stall_switch;						//堵转保护开关
	//模式
	...
	uint8_t			goal_disable;		//目标失能(由信号输入)
	uint8_t			goal_brake;			//目标刹车(由信号输入)
	//软目标
	...
	uint8_t			soft_disable;		//软失能
	uint8_t			soft_brake;			//软刹车
	uint8_t			soft_new_curve;	//新软目标曲线
	//输出
...
void Motor_Control_SetStallSwitch(uint8_t _switch);		//堵转保护开关
