...
/**
  ...
  * @retval NULL
	* 以cyc为一个周期的求差
  * 例如a = 16384, b = 1,cyc = 16384, sub_data = 16383>(16384/2),sub_data = sub_data - 16384 = -1
**/
...
/**
  ...
  * @param  NULL
  * @retval NULL
	* ie. a = 16384, b = 2, cyc = 16384;sub_data = 16384 - 1 = 16382 > (cyc/2),ave_data = (16384 + 2) / 2;因为ave_data >= cyc/2，ave_data = ave_data - cyc/2
**/
...
void Calibration_Data_Check(void)
{
	...
	for(count=1; count<Move_Step_NUM; count++)
	{
		...
		if(D_ABS(sub_data) > (CALI_Gather_Encode_Res * 3 / 2))	{	encode_cali.error_code = CALI_Error_Average_Continuity;	encode_cali.error_data = count;	return;	}	//两次数据差过大
		if(D_ABS(sub_data) < (CALI_Gather_Encode_Res * 1 / 2))	{	encode_cali.error_code = CALI_Error_Average_Continuity;	encode_cali.error_data = count;	return;	}	//两次数据差过小
...
//	uint16_t	data_u8;
	...
	//motor_control.stall_flag = 1;	//触发堵转保护,即校准后禁用运动控制
	
	//清理校准信号
	encode_cali.state = CALI_Disable;
	encode_cali.trigger =0;			//清除校准触发
}
